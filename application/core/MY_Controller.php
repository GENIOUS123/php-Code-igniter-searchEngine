...
exit();*/
return redirect('public/User_authonication');
...
$sql ="SELECT `product`.*,`stock`.`productid`,`stock`.`color`,`stock`.`sl_no`,`stock`.`colorcode`,(`stock`.`mrp`) as mrp ,(SELECT SUM(`stock`.`qts`) FROM `stock` WHERE `stock`.`productid` ='$productid') as sumqts,(SELECT SUM(CASE WHEN `sales`.`status` !='Cancelled' THEN `sales`.`qts` ELSE 0 END) FROM `sales` WHERE `sales`.`sale_productid` = `stock`.`productid`) as `sumsaleqts`,(`stock`.`discount_percentage`) as discount_percentage ,(`stock`.`net_output_rate`) as net_output_rate  FROM `product` LEFT JOIN `stock` ON `stock`.`productid` = `product`.`productid`  WHERE `stock`.`productid` ='$productid' ORDER BY `stock`.`sl_no` DESC LIMIT 1";
...
$output= [
             ...
             "discountrate"=>round($y->net_output_rate,1)
             /*"discountrate"=>round($y->net_output_rate,1)*/
